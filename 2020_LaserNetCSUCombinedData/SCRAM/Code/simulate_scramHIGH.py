...
class SCRAMTarget:
    def __init__(self,densLayers,tempLayers,j,k,j_f,k_f,fh,fj,en_VH,en_HR,dx=1e-5):
    ...
    def generateLayers(self):
        ...
        for dens, temp in zip(self.densLayers, self.tempLayers):
            ...
            k_layer_f = (np.log(dens),np.log(temp),np.log(self.en_VH))
            k_layer_f = self.k(k_layer_f)
            k_layer_f = np.exp(k_layer_f)
            j_layer_f = (np.log(dens),np.log(temp),np.log(self.en_VH))
            j_layer_f = self.j(j_layer_f)
            j_layer_f = np.exp(j_layer_f)
            ...
            k_layer_r = (np.log(dens),np.log(temp),np.log(self.en_HR))
            k_layer_r = self.k(k_layer_r)
            k_layer_r = np.exp(k_layer_r)
            j_layer_r = (np.log(dens),np.log(temp),np.log(self.en_HR))
            j_layer_r = self.j(j_layer_r)
            j_layer_r = np.exp(j_layer_r)
        ...
        for i,layer in enumerate(self.layers):
            ...
            #transmission to Von Hamos crystal
                ...
                if layer[1] < 6.4:
                    ...
                    intensity_layer = (layer[3])/layer[2]*(1-trans_layers[i])
